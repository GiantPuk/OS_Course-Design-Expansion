...
|                      User Layer                             |
|  +--------------+  +--------------+  +--------------+       |
|  |   main.c     |  | test_basic.c |  |  plot.py     |       |
|  |  Experiments |  |  Unit Tests  |  |  Visualization        |
|  +------+-------+  +------+-------+  +------+-------+       |
+--------+-----------------+-----------------+---------------+
         |                 |                 |
+--------+-----------------+-----------------+---------------+
...
|  +------+-----------------+-----------------+------+        |
|  |              scheduler.h / simulator.c            |       |
|  |         Unified interface + discrete event engine |       |
|  +--------------------------+------------------------+       |
|                             |                                |
|  +--------------------------+------------------------+       |
|  |         Algorithm Implementation Layer            |       |
|  |  +-----+ +-----+ +-----+ +-----+ +-----+ +-----+ |       |
|  |  | RMS | | EDF | | LLF | | DMS | |FCFS | | SJF | |       |
|  |  +-----+ +-----+ +-----+ +-----+ +-----+ +-----+ |       |
|  +----------------------------------------------------+       |
|                             |                                |
|  +--------------------------+------------------------+       |
|  |         Core Data Layer                           |       |
|  |  rt_types.h  task.h  task.c  utils.h  utils.c     |       |
|  +----------------------------------------------------+       |
+-------------------------------------------------------------+
```
